...
plot_ly(showscale = FALSE) %>% # data will be added seperately since three plots should be combined
  add_surface(z = ~log_IQR_1, colorscale = list(c(0, 1), c('#fc9272', '#de2d26')), opacity = 0.7) %>%  # first plot - alpha = 1.7
  add_surface(z = ~log_IQR_2, colorscale = list(c(0, 1), c('#bcbddc', '#756bb1')),  opacity = 0.7) %>% # first plot - alpha = 1.7 - add opacity to make is easier to see
  add_surface(z = ~log_IQR_3, colorscale = list(c(0, 1), c('#9ecae1', '#3182bd')),  opacity = 0.7) %>% # first plot - alpha = 1.7
  layout(
    title = "Skew and IQRs",
